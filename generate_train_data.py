...
#     original_data = json.load(f)[10:200]  # 取前10条
...
# 读取原始数据
with open('./page0_train_data.json', 'r') as f:
    original_data = json.load(f)[800:1400]
    

for data in original_data:
    data["CoTQuery"] = process_batch(data["raw_data"])
    print(f"\n\n\n{data["id"]}: {data["CoTQuery"]}")
    print(f"\n\n\n{data["id"]}: is finish")

with open('./train_data_demo3.json', 'w') as f:
    json.dump(original_data, f, indent=2, ensure_ascii=False)
